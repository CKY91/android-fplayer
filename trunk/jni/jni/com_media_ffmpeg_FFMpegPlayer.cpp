...
{
	__android_log_print(ANDROID_LOG_ERROR, TAG, "setMediaPlayer");
    MediaPlayer* old = (MediaPlayer*)env->GetIntField(thiz, fields.context);
    if (old != NULL) {
		__android_log_print(ANDROID_LOG_ERROR, TAG, "freeing old mediaplayer object");
		free(old);
	}
...
com_media_ffmpeg_FFMpegPlayer_prepare(JNIEnv *env, jobject thiz)
{
	...
	__android_log_print(ANDROID_LOG_ERROR, TAG, "com_media_ffmpeg_FFMpegPlayer_prepare-----------------------------------------------");
    MediaPlayer* mp = getMediaPlayer(env, thiz);

    if (mp == NULL ) {
    	__android_log_print(ANDROID_LOG_ERROR, TAG, "com_media_ffmpeg_FFMpegPlayer_prepare  11111  jniThrowException-----------------------------------------------");
        jniThrowException(env, "java/lang/IllegalStateException", NULL);
        return;
    }

    __android_log_print(ANDROID_LOG_ERROR, TAG, "com_media_ffmpeg_FFMpegPlayer_prepare2-----------------------------------------------");
